...
const EpisodeCard = ({ item }: Props) => {
  const image = item.still_path
    ? `${tmdbImageUrl}/${item.still_path}`
    : "/assets/no_image.jpg";

  const router = useRouter();
  const handleNavigate = () => {
    router.push(`${item.season_number}/episode/${item.episode_number}`);
  };
  ...
  return (
    <Card className="w-full flex items-center p-4 gap-4 shadow-md">
      ...
      <CardContent className="flex-1 ">
        ...
        <p className="text-sm text-gray-500">{item.episode_number} Episode</p>
